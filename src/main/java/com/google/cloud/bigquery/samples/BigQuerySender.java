...
public class BigQuerySender {
  ...
  //
  /////////////////////////
  private static final String PROJECT_ID = "helpful-surge-498";
  ...
  public boolean googleContainsTrigram(String term) {
      String querySql = "SELECT ngram FROM publicdata:samples.trigrams where ngram == \""+term + "\"";
      try {
          ...
          return displayQueryResults(bigquery, PROJECT_ID, completedJob);
      } catch(Exception e) {
          e.printStackTrace();
      }
      return true;
...
// SELECT ngram FROM [publicdata:samples.trigrams] WHERE ngram CONTAINS "dynamic programming"

      // Print out available datasets in the "publicdata" project to the console
    listDatasets(bigquery, "publicdata");

    // Start a Query Job
    String querySql = "SELECT ngram FROM publicdata:samples.trigrams where ngram == \"standard template library\"";
    JobReference jobId = startQuery(bigquery, PROJECT_ID, querySql);
...
//    System.out.format("\nJob ID of Query Job is: %s\n", jobId.getJobId());
   ...
   * @throws java.io.IOException
   */
  private static boolean displayQueryResults(Bigquery bigquery,
  ...
  private static GoogleClientSecrets loadClientSecrets(String clientSecretsLocation) {
    try {
        InputStream is = new FileInputStream(clientSecretsLocation);
    //  clientSecrets = GoogleClientSecrets.load(new JacksonFactory(),
     //     BigQuerySender.class.getResourceAsStream(clientSecretsLocation));
        clientSecrets =  GoogleClientSecrets.load(new JacksonFactory(), is);
    } catch (Exception e) {
